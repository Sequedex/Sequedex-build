...
binary release or the directory where the Sequedex project has been cloned or 
...
If you have already downloaded and uncompressed a Sequedex binary release, 
then go directly to step 4.

Step 1.

Clone (or checkout) Sequedex project from github into <git-top-level>.  

You may also need to download the most recent Sequedex build release (see steps
2 and 3). 
...
The easiest way to populate the data directory is to copy the files in the
data directory of a binary release of Sequedex.  For example, 
from the command line on Linux or Mac:

cp -rp <path-to-release-dir>/data/* <git-top-level>/data
...
required jar files. The easiest way to populate this subdirectory
is to copy the files from the thirdPartyJarFiles directory in a compatible 
binary release of Sequedex.  For example, from the command line on Linux or Mac:

cp -rp <path-to-release-dir>/thirdPartyJarFiles/* <git-top-level>/thirdPartyJarFiles
...
If files from data and thirdPartyJarFiles directories were copied from the 
current binary release, data subdirectory should contain:
...
The binary release of Sequedex includes the directory sequedex in <git-top-level>.
This directory contains everything needed to run Sequedex and can be copied or
moved to another location for production use.  The sequedex runtime 
directory is built by executing the build script <git-top-level/build_standalone.xml.
If <git-top-level>/sequedex directory already exists, the build script will 
delete it before building a new version.  This script can be executed using 
Apache Ant.  If Apache Ant is not already installed, it can be found at 
